  ...
  try {
    ...
    const [tasksResult] = await pool.query(
      `SELECT t.*, f.name as factory_name, f.latitude, f.longitude, f.address 
       ...
       WHERE t.status = 'pending' AND t.scheduled_date >= ? AND t.scheduled_date <= ? 
    ...
    await pool.query(
      'INSERT INTO schedules (id, week_start, week_end, total_cost, total_travel_time, workload_variance, status) VALUES (?, ?, ?, ?, ?, ?, ?)',
...
export const getSchedule = async (req: Request, res: Response) => {
  try {
    ...
    const [scheduleResult] = await pool.query('SELECT * FROM schedules WHERE id = ?', [id]);
